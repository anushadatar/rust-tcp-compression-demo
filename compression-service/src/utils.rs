...
pub fn compress_command(input: &mut message::Message, output: &mut message::Message) {
    // TODO Create and set a payload here.
    let output_size = compress::compress_message(input.payload(), &mut output.payload());
    // TODO Error checking and update satats.
}

pub fn validate_header_and_set_response_code(
    input: &mut message::Message,
    output: &mut message::Message,
) {
    // TODO clean this up!
...
/// TODO DOCSTRING
pub fn validate_and_set_payload_length(
    input: &mut message::Message,
    output: &mut message::Message,
) {
...
mod tests {
    ...
    fn test_ping() {
        let received_data = [
            83u8, 84, 82, 89, 0, 0, 0, definitions::RequestCode::Ping as u8];
